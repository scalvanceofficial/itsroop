          ...
          <div class="wd-form-order">
            <div class="order-head d-flex justify-content-between align-items-center">
              ...
              <div class="content ms-auto">
                <a href="{{ route('frontend.orders.pdf.download', $order->id) }}" class="btn btn-danger" target="_blank" title="Download PDF">
                    ...
                    <ul class="timeline">
                      @if (!empty($tracking_activities))
                        @php
                          ...
                          $delivery_statuses = ['delivered', 'out for delivery', 'shipped', 'picked up'];
                        @endphp
                        @if ($order->shiprocket_status != 'Delivered')
                          @foreach ($tracking_activities as $tracking_activity)
                            @php
                              $current_status = strtolower($tracking_activity['sr-status-label']);
                            @endphp

                            @if ($current_status !== 'na' && in_array($current_status, $delivery_statuses) && ($current_status !== 'in transit' || !$in_transit_shown))
                              <li>
                        ...
                        @else
                          @foreach ($delivery_statuses as $delivery_status)
                            <li>
                              ...
                              <div class="timeline-box">
                                <a class="timeline-panel" href="javascript:void(0);">
                                  <div class="text-2 fw-6">{{ strtoupper($delivery_status) }}</div>
                                </a>
                              </div>
                            </li>
                          @endforeach
                ...
                <div class="widget-content-inner">
                  ...
                  @foreach ($order->products as $order_product)
                    ...
                    <div class="order-head">
                      ...
                      <div class="content">
                        <div class="text-2 fw-6">
                          <a href="{{ route('frontend.products.product-details', $order_product->product->slug) }}">
                            ...
                            <div class="mt_4">
                              ...
                              @elseif($returned_product->status == 'REFUND_PROCESSED')
                                <span class="badge bg-primary text-white">Refund
                                  Processed</span>
                                <div class="mt-2 px-2 py-1 bg_f5f5ec border rounded small text-muted" style="max-width: 350px;">
                              ...
                              @elseif ($returned_product->status == 'REFUND_COMPLETED')
                                ...
                                <div class="mt-2 px-2 py-1 bg_f5f5ec border rounded small text-muted" style="max-width: 500px;">
                    ...
                    @if ($order->created_at->diffInHours() < 24 && $order->shiprocket_status == 'NEW' && $order->shiprocket_status != 'Cancelled')
                      <div class="alert alert-info mb-4">
                        ...
                        <p>You can cancel your order within 24 hours of placing it. If you have any
                          ...
                          <a href="mailto:{{ config('app.admin_email') }}" class="text-decoration-underline text-custom-primary">{{ config('app.admin_email') }}</a>
                        </p>
                        <p>Please note that once an order is shipped, it cannot be cancelled.</p>
                        <div class="d-flex justify-content-end">
                          <a href="javascript:void(0);" class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#cancelOrderModal">
                            Cancel Order
                          </a>
                        </div>
                      </div>
                    @elseif($order->shiprocket_status == 'Cancelled')
                      <div class="alert alert-warning mb-4">
                        ...
                        <p>Your order has been successfully cancelled. For further assistance,
                          contact us at
                          <a href="mailto:{{ config('app.admin_email') }}" class="text-decoration-underline text-custom-warning">{{ config('app.admin_email') }}</a>
                        </p>
                      </div>
                    @else
                      <div class="alert alert-secondary mb-4">
                        ...
                        <p>Order cancellation is not available. If you have any questions, please
                          contact us at
                          <a href="mailto:{{ config('app.admin_email') }}" class="text-decoration-underline text-custom-secondary">{{ config('app.admin_email') }}</a>
                        </p>
      ...
      <div class="modal-content border-0 rounded-3 shadow-sm">
        ...
        <form action="{{ route('frontend.orders.cancel', $order->route_key) }}" method="POST" id="cancelOrderForm">
          ...
          <div class="modal-body">
            ...
            <textarea name="cancellation_reason" rows="3" class="form-control border-secondary rounded-2" placeholder="Reason for cancellation"></textarea>
          </div>
